...
This probe targeted the catalog-declared but still-unmapped DPID set for `rju4K7`:
...
- `13 = 2` did not reproduce that delayed power-off behavior in the focused rerun

Working interpretation:

- `13` is likely not a no-op
- `13 = 1` appears to map to some delayed shutdown or delayed off-like behavior
...
- mode-specific behavior not reached by this probe
- write-only behavior
- delayed behavior not visible in the immediate query window
...
- catalog-declared for `rju4K7`
- unverified as user-facing Home Assistant capabilities
- not mapped to first-class light behavior in the current integration
